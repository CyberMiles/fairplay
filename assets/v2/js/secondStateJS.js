...
var modifyTemplate = (index, item) => {
        ...
        var playUrl = item.dappVersion + "/play.html?contract=" + item.contractAddress;
        ...
        $(".card").each((id, card) => {
            sub = $(card).find(".block-number").text().split(" ").slice(-1)[0] - currentHeight // English: block number: xxx || Chinese: 块高: xxx
            if (difference == 0){
